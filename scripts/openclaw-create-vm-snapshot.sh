...
[[ "$resource_group_id" =~ ^/subscriptions/[^/]+/resourceGroups/[^/]+$ ]] || {
  printf 'Unexpected resource-group resource ID.\n' >&2
  exit 1
}
subscription_id="${resource_group_id#/subscriptions/}"
subscription_id="${subscription_id%%/resourceGroups/*}"
if [[ -n "${OPENCLAW_AZURE_SUBSCRIPTION_ID:-}" &&
  "${OPENCLAW_AZURE_SUBSCRIPTION_ID,,}" != "${subscription_id,,}" ]]; then
  printf 'Configured Azure subscription does not contain the target resource group.\n' >&2
  exit 1
fi
...
az snapshot create \
  ...
  --incremental true \
  --tags purpose=openclaw-automated-backup retention=3 createdBy=openclaw-vm-snapshot \
...
jq -e --arg name "$snapshot_name" --arg source "${os_disk_id,,}" '
  ...
  (try (.creationData.sourceResourceId | ascii_downcase) catch null) == $source and
  .tags.purpose == "openclaw-automated-backup" and
  .tags.retention == "3" and
...
jq -S --arg source "${os_disk_id,,}" '
  [.[] |
    select(.name | test("^openclaw-auto-daily-[0-9]{8}T[0-9]{6}Z$")) |
    select(.provisioningState == "Succeeded" and .incremental == true and
      (try (.creationData.sourceResourceId | ascii_downcase) catch null) == $source and
      .tags.purpose == "openclaw-automated-backup" and .tags.retention == "3" and
      .tags.createdBy == "openclaw-vm-snapshot") |
...
jq -e --arg name "$snapshot_name" --arg source "${os_disk_id,,}" '
  length >= 1 and any(.[]; .name == $name and .provisioningState == "Succeeded" and
    .incremental == true and (try (.sourceResourceId | ascii_downcase) catch null) == $source and
    .tags.purpose == "openclaw-automated-backup" and .tags.retention == "3" and
    .tags.createdBy == "openclaw-vm-snapshot")
