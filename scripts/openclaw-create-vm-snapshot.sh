...
readonly name_prefix='openclaw-auto-weekly-'
readonly retention="${OPENCLAW_SNAPSHOT_RETENTION:-2}"
[[ "$retention" =~ ^[1-9][0-9]*$ ]] || {
  printf 'OPENCLAW_SNAPSHOT_RETENTION must be a positive integer.\n' >&2
  exit 64
}
...
[[ -n "$resource_group" ]] || { printf 'OPENCLAW_AZURE_RESOURCE_GROUP is required.\n' >&2; exit 64; }
...
[[ "$snapshot_name" =~ ^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$ ]] || exit 1
az snapshot create \
  ...
  --incremental true \
  --tags purpose=openclaw-automated-backup retention="$retention" createdBy=openclaw-vm-snapshot \
...
jq -e --arg name "$snapshot_name" --arg source "${os_disk_id,,}" --arg retention "$retention" '
  .name == $name and
...
jq -S --arg prefix "$name_prefix" '
  [.[] | select(.name | test("^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$")) |
...
jq -e --arg name "$snapshot_name" --arg source "${os_disk_id,,}" --arg retention "$retention" '
  length >= 1 and
  all(.[]; .name | test("^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$")) and
  all(.[]; .provisioningState == "Succeeded" and
    .incremental == true and ((.sourceResourceId|ascii_downcase) == $source) and
    .tags.purpose == "openclaw-automated-backup" and .tags.retention == $retention and
    .tags.createdBy == "openclaw-vm-snapshot") and
  any(.[]; .name == $name)
...
while IFS= read -r encoded; do
  ...
  [[ "$old_name" =~ ^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$ ]] || exit 1
...
jq -S '[.[] | select(.name | test("^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$")) |
  {name,id,timeCreated,provisioningState,incremental,sourceResourceId:.creationData.sourceResourceId,tags}] |
  sort_by(.timeCreated,.name) | reverse' "$tmpdir/after.json" > "$tmpdir/automated-after.json"
jq -e --arg name "$snapshot_name" --arg source "${os_disk_id,,}" --arg retention "$retention" '
  length <= $retention and
  all(.[]; .name | test("^openclaw-auto-weekly-[0-9]{8}T[0-9]{6}Z$")) and
  all(.[]; .provisioningState == "Succeeded" and .incremental == true and
    ((.sourceResourceId|ascii_downcase) == $source) and
    .tags.purpose == "openclaw-automated-backup" and
    .tags.retention == $retention and .tags.createdBy == "openclaw-vm-snapshot") and
  any(.[]; .name == $name)
